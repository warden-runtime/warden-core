...
| `TOOL_INVOKE_FAILED` with Pydantic `list_type` / `dict_type` on a local model | Model emitted stringified JSON for an array/object field, or args are wrong past the coercion depth | Warden coerces common cases automatically ([Configuration → Tool argument coercion](configuration.md#tool-argument-coercion)); if it still fails, inspect `error_details.message`, confirm MCP `inputSchema` types, and check for mistakes deeper than two nesting levels |
