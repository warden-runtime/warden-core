...
On **`react` submit** steps, tool return text that matches failure heuristics normally raises `TOOL_OUTPUT_ERROR` and fails the step. A narrow set of **recoverable** mismatches (default: `old_text` miss/ambiguous/empty, write “file exists”, missing path, patch-apply text failures, directory/file-type mistakes, and Pydantic tool-arg validation messages) are instead appended as tool-role messages — with a one-line recovery hint — so the model can re-read and retry within `max_turns`. MCP `CallToolResult.isError` payloads are normalized to `Error:` and follow the same path. Recoverable **invoke exceptions** (e.g. `IsADirectoryError`) are converted to `Error:` strings instead of aborting with `TOOL_INVOKE_FAILED`; infrastructure failures (MCP transport, connection/timeout, docker/container death — including when nested inside an `ExceptionGroup`) still hard-fail the step. `apply_patch_sandbox` JSON rejects (`{"ok": false, "phase": ...}`) already stay in-transcript; they get the same hint. `MCP error` / invalid-argument style failures stay hard. Enterprise plugins may override via `ToolLifecycleHooks.tool_output_is_recoverable`.
