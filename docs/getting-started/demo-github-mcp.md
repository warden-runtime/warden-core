...
| `no_submit_call` | ReAct step exhausted turns without `_submit` — on text-only exit (`model_text_exit`), check `last_assistant_content` in `warden show step … --json`; use `last_tool_errors` only for real MCP failure output |
| `TOOL_INVOKE_FAILED` (tool arg validation) | Model sent wrong types for MCP tool args — Warden admits common Ollama stringification ([Configuration → LLM JSON admission](configuration.md#llm-json-admission)); persistent failures may need a stronger model or shallower tool schemas |
